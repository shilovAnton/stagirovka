...
/* таблица переходная "Адрес клиента" */
CREATE TABLE address_client (
    id_address_client                       NUMBER
        GENERATED AS IDENTITY,
    id_client                               NUMBER,
    id_address                              NUMBER,
    CONSTRAINT address_client PRIMARY KEY ( address_client )
);
...
/* таблица "Пакеты" */
CREATE TABLE packege_orders (
    id_packege_orders                    NUMBER
        GENERATED AS IDENTITY,
    id_courier                          NUMBER,
...
COMMENT ON TABLE packege_orders IS
    'Таблица "Пакет заказов" рейс курьера';

COMMENT ON COLUMN packege_orders.id_packege_orders IS
    'Идентификационный номер';

COMMENT ON COLUMN packege_orders.id_courier IS
    'id курьера';

COMMENT ON COLUMN packege_orders.detaTimeC_packege_create IS
    'дата и время создания пакета заказов';

COMMENT ON COLUMN packege_orders.detaTimeC_packege_end IS
